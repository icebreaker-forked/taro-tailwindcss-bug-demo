...
export default () => {
	...
	return (
		<View className={"service_container"}>
			<View className={"fontSize18 mt20 fontWeightBold"}>123123132</View>
			...
			{List.map((item, index) => {
				...
				if (item.subTitle) {
					return <View className={subTitleClass}>{item.subTitle}</View>;
				}
				if (item.text) {
					return (
				...
				if (item.lastText) {
					return <View className={lastTextClass}>{item.lastText}</View>;
				}
			})}
		</View>
	);
